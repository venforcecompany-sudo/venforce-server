...
function normalizarIdImport(valor) {
...
function parseNumeroPlanilha(valor) {
...
function processarWorksheetFormatoFixo(worksheet) {
  const rows = XLSX.utils.sheet_to_json(worksheet, {
    header: 1,
    defval: ""
  });

  // ignorar primeira linha (header)
  const data = rows.slice(1);

  const dadosProcessados = data.map((row) => {
    return {
      id: normalizarIdImport(row?.[0]),
      custo: parseNumeroPlanilha(row?.[1]),
      imposto: parseNumeroPlanilha(row?.[2])
    };
  });

  const validos = dadosProcessados.filter((r) => r.id);

  if (!validos.length) {
    throw new Error(
      `Formato inválido.\n\n` +
        `A planilha deve conter:\n\n` +
        `Coluna A: ID (somente número, sem MLB)\n` +
        `Coluna B: Custo\n` +
        `Coluna C: Imposto\n`
    );
  }
  ...
  for (const r of validos) {
    resultado[r.id] = {
      custo_produto: r.custo,
      imposto_percentual: r.imposto,
      taxa_fixa: 0
    };
...
function senhaEhHash(senhaSalva) {
  const texto = String(senhaSalva || "");
  return (
    texto.startsWith("$2a$") ||
    texto.startsWith("$2b$") ||
    texto.startsWith("$2y$")
...
async function validarSenha(senhaDigitada, senhaSalva) {
  if (!senhaSalva) return false;

  if (senhaEhHash(senhaSalva)) {
    return bcrypt.compare(String(senhaDigitada || ""), String(senhaSalva));
  }
...
function parsePlanilhaParaResultado(rows) {
  ...
  for (const row of rows) {
    ...
    let impostoPercentual = numeroSeguro(
      obterValorColuna(row, [
        "imposto_percentual",
        "IMPOSTO_PERCENTUAL",
        "imposto",
...
function lerPlanilhaParaJson(filePath, originalName) {
  ...
  const { resultado } = processarWorksheetFormatoFixo(worksheet);
  return resultado;
}
...
app.post("/login", async (req, res) => {
  try {
    ...
    const valid = await validarSenha(password, senhaSalva);
...
app.post("/bases/:cliente/importar", authMiddleware, upload.single("arquivo"), (req, res) => {
  ...
  try {
    ...
    const user = req.user;
    const confirmar =
      String(req.body?.confirmar || req.body?.confirm || "").trim().toLowerCase() === "true";
    ...
    const primeiraAba = workbook.SheetNames[0];
    const worksheet = primeiraAba ? workbook.Sheets[primeiraAba] : null;

    if (!worksheet) {
      return res.status(400).json({
        ok: false,
        erro: "A planilha não possui abas válidas"
      });
    }

    const { resultado, preview, total, idsDetectados } = processarWorksheetFormatoFixo(worksheet);
    ...
    if (!confirmar) {
      return res.json({
        ...
        total,
        idsDetectados,
        mensagem: "Preview gerado. Envie confirmar=true para importar."
      });
    }
